...
export default {
   ...
   */
    function createDragger (el) {
      ...
      let draggerStyler = document.createElement('div')
    ...
    function updateDragger (el, options) {
      ...
      state.dragger.style.height =
        parseInt(Math.round(state.barHeight)) + 'px'
      ...
      if (options.withScrollingClasses) {
        ...
        if (state.scrollingClassTimeout) {
        ...
        if (state.scrollingPhantomClassTimeout) {
    ...
    function hideScrollbarUsingPseudoElement (el) {
      let state = getState(el)
      let idName = 'vuebar-pseudo-element-styles'
      let selector = '.' + state.config.el2Class + '::-webkit-scrollbar'
      ...
      for (let i = 0, l = sheet.rules.length; i < l; i++) {
    ...
    function preventParentScroll (el, event) {
      ...
      let scrollDist = state.el2.scrollHeight - state.el2.clientHeight
      let scrollTop = state.el2.scrollTop
   ...
   */
    ...
    function documentMouseup (el) {
      ...
      return function (/* event */) {
        ...
        if (state.config.unselectableBody) {
    ...
    function barMousedown (el) {
      ...
      return function (event) {
        ...
        if (state.draggingPhantomClassTimeout) {
  ...
  \*------------------------------------ */
    function initScrollbar (el, kwargs) {
      ...
      if (el._vuebarState) {
        ...
        Vue.util.warn(
          '(Vuebar) Tried to initialize second time. If you see this please ' +
          'create an issue on https://github.com/DominikSerafin/vuebar with ' +
          'all relevent debug information. Thank you!'
        )
        return false
      }
      ...
      options = kwargs.value ? kwargs.value : kwargs || {}
      ...
      // dragger enabled?
      let elNativeScrollbarWidth = getNativeScrollbarWidth(
        el.firstElementChild
      )
      let overlayScrollbar = elNativeScrollbarWidth === 0
  ...
  \*------------------------------------ */
    function detectBrowser () {
      ...
      function getIEVersion () {
        let match = window.navigator.userAgent.match(
          /(?:MSIE |Trident\/.*; rv:)(\d+)/
        )
      ...
      // internet explorer
      let ie8 = getIEVersion() === 8
      let ie9 = getIEVersion() === 9
      let ie10 = getIEVersion() === 10
      let ie11 = getIEVersion() === 11
      let ie = ie8 || ie9 || ie10 || ie11

      // is it mobile browser?
      // regex below thanks to http://detectmobilebrowsers.com/
      let uaOrVendor = ua || vendor || window.opera
      let mobile =
